...
const generate = (latexCode) => {
  return new Promise((resolve, reject) => {
    ...
    const texFilePath = path.join(tmpDir.name, `${timestamp}.tex`);
    const pdfFilePath = path.join(tmpDir.name, `${timestamp}.pdf`);

    fs.writeFileSync(texFilePath, latexCode);

    exec(
      `pdflatex -interaction=nonstopmode -output-directory=${tmpDir.name} ${texFilePath}`,
      (error, stdout, stderr) => {
        if (error) {
